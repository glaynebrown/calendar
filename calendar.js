...
const Calendar = {
  ...
  openEventModal(event, dateStr) {
    ...
    const body = `
      ...
      <div class="field-row">
        <div class="field">
          ...
          <select id="ev-owner">${editablePeople.map(p => `<option value="${p.id}" ${event && event.ownerId === p.id || (!event && p.id === userId) ? 'selected' : ''}>${p.name}</option>`).join('')}</select>
        </div>
        <div class="field">
      ...
      <div class="field" id="ev-people-picker-field">
        <div id="ev-people-picker" class="people-picker">
    ...
    openModal(body, root => {
      ...
      const peoplePickerField = root.querySelector('#ev-people-picker-field');
      const peoplePicker = root.querySelector('#ev-people-picker');
      peoplePickerField.classList.toggle('hidden', privateCb.checked);
