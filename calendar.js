...
function isEventVisible(event, currentUserId, viewPeopleIds) {
  if (!viewPeopleIds.includes(event.ownerId)) return false;
...
const Calendar = {
  ...
  renderCategoryHeader(menu) {
    ...
    addRow.innerHTML = `
      <input type="text" id="vm-new-category" placeholder="Category name" style="flex:1; padding:6px 8px; border-radius:8px; border:0.5px solid var(--border-strong); background:var(--surface-2); color:var(--text); font-size:13px;">
  ...
  renderDayView(everyone) {
    ...
    dayEvents.forEach(ev => {
      const color = this.colorForEvent(ev, userId);
      const owner = everyone ? Store.getPerson(ev.ownerId) : null;
      const row = document.createElement('button');
      row.type = 'button';
      ...
      row.innerHTML = `
        ...
        ${owner ? `<span class="muted day-event-owner">${escapeHTML(owner.name)}</span>` : ''}
      `;
      row.addEventListener('click', e => {
        e.stopPropagation();
        if (ev.isBirthday) this.openBirthdayModal(ev.ownerId, ev.birthdayId);
  ...
  openEventModal(event, dateStr) {
    ...
    const body = `
      ...
      <div class="field" id="ev-reminder-field">
        ...
        <div id="ev-reminder-menu" class="repeat-menu hidden">
          ...
          <div id="ev-reminder-custom-row" class="hidden" style="display:flex; gap:6px; padding:6px 10px 4px;">
            <input type="number" id="ev-reminder-custom-amount" min="1" placeholder="Amount" style="width:70px; padding:6px 8px; border-radius:8px; border:0.5px solid var(--border-strong); background:var(--surface-2); color:var(--text); font-size:13px;">
            <select id="ev-reminder-custom-unit" style="flex:1; padding:6px 8px; border-radius:8px; border:0.5px solid var(--border-strong); background:var(--surface-2); color:var(--text); font-size:13px;">
      ...
      <div class="field-row">
        <div class="field">
          <select id="ev-owner">${people.map(p => `<option value="${p.id}" ${event && event.ownerId === p.id || (!event && p.id === userId) ? 'selected' : ''}>${p.name}</option>`).join('')}</select>
        </div>
        <div class="field">
      ...
      <div class="checkbox-row">
        ...
        <button type="button" class="link-btn" id="ev-share-toggle">Share with specific people</button>
      </div>
      <div id="ev-people-picker" class="people-picker hidden">
        ${people.map(p => `<label><input type="checkbox" value="${p.id}" ${event && (event.customPeople || []).includes(p.id) ? 'checked' : ''}> ${p.name}</label>`).join('')}
      </div>
      <div class="btn-row">
    ...
    openModal(body, root => {
      ...
      let reminderVal = event && event.reminder != null ? event.reminder : null;
      ...
      let repeatVal = type === 'custom' ? 'custom' : (type === 'recurring' && event ? event.recurrence.freq : 'single');
      ...
      const shareToggle = root.querySelector('#ev-share-toggle');
      const peoplePicker = root.querySelector('#ev-people-picker');
      ...
      function refreshRepeatUI() {
        ...
        repeatBtn.title = `Repeat: ${REPEAT_LABELS[repeatVal]}`;
      ...
      shareToggle.addEventListener('click', () => {
        peoplePicker.classList.toggle('hidden');
        if (!peoplePicker.classList.contains('hidden')) privateCb.checked = false;
      });
      privateCb.addEventListener('change', () => {
        if (privateCb.checked) peoplePicker.classList.add('hidden');
      });
          ...
          openMultiDatePicker([], picked => {
            picked.forEach(d => {
              Store.addEvent({
                ...
                ownerId: event.ownerId, category: event.category,
                visibility: event.visibility, customPeople: event.customPeople || [],
                notes: event.notes, attachment: event.attachment, reminder: event.reminder,
                order: Date.now(),
      ...
      root.querySelector('#ev-save').addEventListener('click', () => {
        ...
        const ownerId = root.querySelector('#ev-owner').value;
        const category = root.querySelector('#ev-category').value.trim() || null;
        if (category) Store.addCategory(category);

        const customPeopleChecked = Array.from(peoplePicker.querySelectorAll('input:checked')).map(i => i.value);
        let visibility = 'shared';
        let customPeople = [];
        if (privateCb.checked) visibility = 'private';
        else if (customPeopleChecked.length) { visibility = 'custom'; customPeople = customPeopleChecked; }
        ...
        let newEvent = {
          ...
          title, date, endDate, time, endTime, ownerId, category, visibility, customPeople,
          notes, attachment: attachmentVal, reminder: reminderVal,
        ...
        if (repeatVal === 'single') {
          ...
          const rule = { freq: repeatVal, interval: 1, until: null, count: null };
