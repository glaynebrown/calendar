...
const Calendar = {
  ...
  // Pure date math, no Store/auth dependency -- paints the current month's
  // grid (dates + today highlight, no events) the instant the page loads,
  // before Firebase Auth has resolved whether anyone's signed in. Nothing
  // here binds listeners or touches Store, so it's safe to call before
  // init() and harmless to have its output fully overwritten by the real
  // render() the moment sign-in resolves a few hundred ms later.
  renderSkeleton() {
    const now = new Date();
    document.getElementById('month-label').textContent =
      now.toLocaleString('default', { month: 'long', year: 'numeric' });
    const gridDates = getGridDates(now.getFullYear(), now.getMonth());
    ...
    gridDates.forEach(d => {
      ...
      cell.className = 'day-cell' + (d.getMonth() !== now.getMonth() ? ' outside' : '') + (formatISO(d) === todayStr ? ' today' : '');
  ...
  openEventModal(event, dateStr) {
    ...
    const body = `
      ...
      <div class="field" id="ev-owner-field">
        <label>Participants</label>
        <button type="button" class="time-field-btn" id="ev-owner-btn">${icon('users')}<span class="tf-text">${editablePeople.filter(p => initialParticipantIds.includes(p.id)).map(p => p.name).join(', ')}</span></button>
        <div id="ev-owner-menu" class="repeat-menu hidden">
    ...
    openModal(body, root => {
      ...
      const ownerBtn = root.querySelector('#ev-owner-btn');
      const ownerMenu = root.querySelector('#ev-owner-menu');
      function refreshOwnerBtn() {
        const names = editablePeople
          .filter(p => ownerMenu.querySelector(`input[value="${p.id}"]`).checked)
          .map(p => p.name);
        ownerBtn.querySelector('.tf-text').textContent = names.join(', ');
      }
      ownerBtn.addEventListener('click', () => ownerMenu.classList.toggle('hidden'));
      ownerMenu.querySelectorAll('input').forEach(cb => cb.addEventListener('change', () => {
        // At least one participant must stay checked -- an event with nobody
        // on it doesn't make sense, and the save handler needs at least one
        // id to fall back to as the owner.
        if (!ownerMenu.querySelectorAll('input:checked').length) cb.checked = true;
        refreshOwnerBtn();
      }));
