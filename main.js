...
const App = {
  ...
  init() {
    ...
    // Painted immediately (not waiting on anything async) so the calendar
    // itself is what's on screen from the first frame -- firebase.auth()'s
    // onAuthStateChanged below always fires asynchronously, even with an
    // already-cached session, so without this there'd be a gap (a loading
    // screen, or worse, a blank grid) before real data shows up. This is
    // pure date math with no Store/auth dependency, so it can run before
    // sign-in resolves; onSignedIn's real render silently overwrites it.
    Calendar.renderSkeleton();
  ...
  async onSignedIn(user) {
    // The loading screen is only shown here if we're coming from an actual
    // sign-in action (whoami-screen was visible -- someone just typed
    // credentials and hit Continue). On a normal cold boot with an
    // already-cached session, whoami-screen was never shown -- the calendar
    // skeleton from init() is already on screen, so popping up "Loading
    // your calendar..." here would just reintroduce the flash this is meant
    // to avoid. Store's live sync silently repaints the skeleton with real
    // data once startSync resolves either way.
