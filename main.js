...
const App = {
  ...
  async maybeHandleInvite() {
    ...
    } else if (invite.type === 'household_member') {
      const h = Store.getHouseholds().find(h => h.id === invite.householdId);
      description = `Join the household "${h ? h.name : ''}"?`;
    } else if (invite.type === 'household_connect') {
      const h = Store.getHouseholds().find(h => h.id === invite.householdId);
      description = `Connect with everyone in "${h ? h.name : ''}"?`;
    }
