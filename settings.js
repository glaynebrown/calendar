...
const Settings = {
  ...
  openModal() {
    ...
    const body = `
      ...
      <div class="settings-section">
        <h3>Connections</h3>
        <p class="muted" style="margin-top:-4px;">Connect with individuals, one at a time.</p>
      ...
      <div class="settings-section">
        <h3>Household</h3>
        <p class="muted" style="margin-top:-4px;">A named group of your connections, so one invite link can connect multiple people at once.</p>
    ...
    openModal(body, root => {
      ...
      function renderHouseholds() {
        ...
        Store.getHouseholdsFor(userId).forEach(h => {
          ...
          const memberNames = h.memberIds.map(id => { const a = Store.getAccount(id); return a ? a.name : '?'; }).join(', ');
          wrap.innerHTML = `
            <p style="font-weight:500;margin:0 0 4px;">${escapeAttr(h.name)}</p>
            <p class="muted" style="margin:0 0 8px;">${escapeAttr(memberNames)}</p>
            <button type="button" class="link-btn" data-action="member">Copy invite to join as a member</button><br>
            <button type="button" class="link-btn" data-action="connect">Copy invite to connect with this household</button>
          `;
          wrap.querySelector('[data-action="member"]').addEventListener('click', async () => {
            const code = await Store.getOrCreateInvite('household_member', userId, h.id);
