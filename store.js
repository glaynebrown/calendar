...
// Limitation: when authoring an event as someone else (the #ev-owner picker),
// this client may not know that person's *other* connections, so a 'shared'
// event created that way is only guaranteed visible to the owner and the
// author, not the owner's full connection list, until it's next edited by
// the owner themselves.
function computeVisibleTo(ownerId, visibility, customPeople) {
  if (visibility === 'private') return [ownerId];
  if (visibility === 'custom') return Array.from(new Set([ownerId, ...(customPeople || [])]));
  // 'shared'
  const currentUserId = Store.getCurrentUserId();
  if (ownerId === currentUserId) {
    return Array.from(new Set([ownerId, ...Store.getConnectedIds(ownerId)]));
  }
  return Array.from(new Set([ownerId, currentUserId]));
}
...
const Store = {
  ...
  startSync(userId) {
    ...
    const pending = new Set(['accounts', 'connections-a', 'connections-b', 'households', 'events', 'categories', 'views']);
    let resolveReady;
  ...
  stopSync() {
    ...
    _cache.events = []; _cache.categories = []; _cache.views = [];
  },
  ...
  async createAccountDoc(userId, name, email) {
    ...
    // Private per-person index of who they're connected to, used only by
    // Firestore security rules (isConnectedTo in firestore.rules) to check
    // "is the requester connected to this event's owner" -- kept separate
    // from the public `accounts` doc for privacy (connections aren't public),
    // and keyed directly by uid (not a composite key) because rules can only
    // reliably exists()/get() a path built from a single direct value, not a
    // concatenated one.
    await db.collection('connectionIndex').doc(userId).set({ ids: [] });
  ...
  addConnection(a, b) {
    ...
    const db = firebase.firestore();
    db.collection('connections').doc(connectionDocId(a, b)).set(conn);
    // Mirrored into connectionIndex (see createAccountDoc) purely so
    // Firestore rules can check "is X connected to Y" via a direct-uid
    // get() instead of a composite key, which rules can't reliably exists()
    // against -- this is a rules implementation detail, not user-facing data.
    db.collection('connectionIndex').doc(a).update({ ids: firebase.firestore.FieldValue.arrayUnion(b) });
    db.collection('connectionIndex').doc(b).update({ ids: firebase.firestore.FieldValue.arrayUnion(a) });
  },
  ...
  removeConnection(userId, otherId) {
    ...
    const db = firebase.firestore();
    db.collection('connections').doc(connectionDocId(userId, otherId)).delete();
    db.collection('connectionIndex').doc(userId).update({ ids: firebase.firestore.FieldValue.arrayRemove(otherId) });
    db.collection('connectionIndex').doc(otherId).update({ ids: firebase.firestore.FieldValue.arrayRemove(userId) });
  ...
  async acceptInvite(code, acceptingUserId) {
    ...
    if (invite.type === 'household_member') {
      this.addHouseholdMember(invite.householdId, acceptingUserId);
      return { ok: true, joinedHousehold: invite.householdId };
    }
    if (invite.type === 'household_connect') {
      ...
      others.forEach(id => this.addConnection(id, acceptingUserId));
      return { ok: true, connectedTo: others };
    }
  ...
  addEvent(event) {
    ...
    const visibleTo = computeVisibleTo(event.ownerId, event.visibility, event.customPeople);
    const full = { ...event, id, visibleTo };
  ...
  updateEvent(id, patch) {
    ...
    if ('visibility' in patch || 'customPeople' in patch || 'ownerId' in patch) {
      merged.visibleTo = computeVisibleTo(merged.ownerId, merged.visibility, merged.customPeople);
    }
